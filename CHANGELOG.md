...
Все значимые изменения проекта «Перископ» фиксируются в этом файле.
...
> **Примечание.** Упоминания сторонних сервисов-аналогов в исторических
> записях этого файла (до 2026-05-15) оставлены без изменений для
> исторической точности — на момент соответствующих релизов формулировки
> в CHANGELOG ссылались на них для контекста. Начиная с записей от
> 2026-05-15 упоминания сторонних сервисов в CHANGELOG, в политике
> конфиденциальности, в условиях использования и в пользовательском
> интерфейсе Сервиса не используются.

---

## v0.52.0 — 2026-06-26 — Полный переход на бренд «Перископ» и домен periscop.pro

### Новые возможности

- **Новый основной домен — [periscop.pro](https://periscop.pro).** Сервис
  переехал на periscop.pro. Прежний адрес продолжает работать как зеркало:
  уже установленные приложения и сохранённые ссылки остаются рабочими —
  ничего переподключать не нужно.

### Улучшения

- **Единый бренд «Перископ» во всём сервисе.** Веб-кабинет, лендинг, письма
  (вход, подтверждение почты, восстановление пароля, SOS-уведомления),
  юридические документы и мобильные приложения теперь используют название
  «Перископ». Приложение ребёнка — «Перископ Ребёнка», приложение
  родителя — «Перископ Родителя».
- **Новые фирменные иконки приложений.** Оба мобильных приложения получили
  новую иконку под бренд «Перископ» — перископ с линзой-«глазом» над картой:
  у приложения ребёнка в бирюзово-зелёной палитре, у родителя — в синей.

### Изменения

- **chore:** внутренние пакеты монорепозитория переведены на бренд
  «Перископ» (`@periscop/*`); конфигурация инфраструктуры и деплоя
  обслуживает оба домена (periscop.pro как основной, прежний — как зеркало).
- **docs:** документация, runbook'и деплоя и карточки RuStore обновлены
  под новый бренд и домен.
- **chore(mobile):** мобильные приложения переведены на новые идентификаторы
  пакета (`pro.periscop.*`); внутренние идентификаторы (namespace, каналы,
  сервисы, префиксы) очищены от прежнего бренда. Публикуются как отдельные
  карточки «Перископ Ребёнка» / «Перископ Родителя» в RuStore.

---

## v0.51.3 — 2026-05-20 — Перископ Родитель: новый бренд родительского приложения

### Улучшения

- **Новый бренд родительского приложения — «Перископ Родитель».**
  Имя в лаунчере, заголовок приложения и упоминания на всех экранах
  (вход, добавление ребёнка, диалоги защиты от удаления) теперь
  «Перископ» вместо «GMD». Иконка прежняя — фирменный pin-маркер,
  у родительского приложения в синей палитре, у приложения ребёнка —
  в зелёной, чтобы их было легко различать. Приложение ребёнка
  («Перископ Ребёнка») уже получило новый бренд в прошлом релизе.

### Изменения

- **docs:** карточка RuStore родительского приложения обновлена под
  новое название «Перископ Родитель», возрастное ограничение 6+.

---

## v0.51.2 — 2026-05-17 — Перископ Ребёнка: новый бренд + персонализация главного экрана

### Новые возможности

- **Имя семьи на главном экране ребёнка** — под приветствием теперь
  отображается название семьи, к которой подключено устройство
  (например, «Ты подключён к семье «Моя семья»»). Раньше был общий
  безличный текст. Помогает ребёнку и взрослому быстро убедиться,
  что устройство привязано к правильному аккаунту.

### Улучшения

- **Новый бренд приложения ребёнка — «Перископ Ребёнка».** Имя в
  лаунчере, заголовке главного экрана и системных диалогах
  разрешений теперь «Перископ» вместо «GMD». На onboarding-экране
  крупная фирменная иллюстрация (щит с pin-маркером и фигурами
  членов семьи) вместо абстрактной иконки. Бренд parent-приложения
  («GMD: родитель») не меняется.
- **Пустое поле ввода кода привязки** — раньше в поле было
  подсказочное «6KDM3B1W», которое из-за крупного шрифта выглядело
  как реальное содержимое и сбивало детей. Теперь поле полностью
  пустое, а пример формата остался отдельной строкой выше.

### Изменения

- **backend:** GET `/child/me` дополнительно возвращает блок
  `family: {id, name}` — нужно для отображения имени семьи в child.
- **docs(claude):** lesson #41 — трёхэтапный release workflow для
  mobile-* (AVD smoke → реальное устройство по Wi-Fi отладке →
  RuStore). Каждый этап блокирующий, нельзя пропустить.

---

## v0.51.1 — 2026-05-16 — Возврат мгновенной доставки «Звук вокруг» и «Сигнал» (фикс orphaned FCM-токена)

### Исправления

- **«Звук вокруг» и «Сигнал» снова работают за секунды, а не минуты.**
  После публикации в RuStore некоторые телефоны ребёнка теряли FCM-токен
  при обновлении приложения, и команды от родителя доходили только через
  fallback-поллинг (минуты). Теперь приложение ребёнка периодически (раз в
  6 часов) автоматически обновляет токен через фоновый WorkManager-сервис,
  и сразу регистрирует его на сервере при ротации — независимо от того,
  открывает ли ребёнок приложение. Backend дополнительно сигналит ребёнку
  через существующий poll-endpoint, если у него отсутствует FCM-токен в
  БД — child-устройство тут же перерегистрирует токен. Закрывает регрессию
  task #68.

### Изменения

- **child:** новый native worker `FcmTokenRefreshWorker` (Kotlin, WorkManager
  periodic 6h) и расширение `AppControlHttp` методом `postFcmToken` —
  обновление токена не требует поднятия Dart-isolate, работает из foreground
  service.
- **child:** `MyFirebaseMessagingService.onNewToken` теперь сразу POST'ит
  новый токен на backend через native HTTP — раньше он только сохранялся
  в `pending_token` SharedPreferences и ждал Dart-side обработки при
  следующем запуске.
- **backend:** в ответ `GET /child/active-block` добавлено поле
  `forceFcmRefresh: boolean` — `true` если у устройства `fcmToken IS NULL`.
...
## v0.50.1 — 2026-05-07 — Восстановление разрешений теперь срабатывает и после ручной переустановки

### Исправления

- **fix(mobile-child): rescue-диалог теперь открывается и после полной переустановки приложения** — в v0.50.0 диалог «Не все разрешения активны» показывался только после in-place auto-update (когда система обновляет APK поверх существующего и сохраняет SharedPreferences). При ручной переустановке (uninstall + install) SharedPreferences с флагом обнулялись и диалог не появлялся, хотя разрешения слетали как обычно. Теперь, если deviceToken жив (онбординг был пройден ранее) и хотя бы одно critical-разрешение выключено — диалог открывается даже без флага. Защита от повторного показа в той же сессии — через статический флаг.
...
- **Открытие `https://gmd.link28rus.ru/`, `/login` или `/register` залогиненным пользователем теперь сразу ведёт в кабинет** — раньше эти страницы всегда показывали форму входа, поэтому при открытии новой вкладки на корне сайта или клик «Войти» в закладке давал ощущение, будто сессию выкинуло. Теперь Server Component на `/`, `/login`, `/register` проверяет refresh-cookie и при наличии редиректит на `/cabinet` (симметрично существующему `/cabinet → /login` при отсутствии cookie).
...
- **Время использования теперь показывается и в карточках hardcoded-системных приложений** (MAX, ru.link28rus.gmd.child) и приложений с `SYSTEM_DEFAULT`-правилом. В v0.46.4 для них подзаголовок жёстко выводил «Системное · нельзя заблокировать» / «По умолчанию разрешено системой» без времени, поэтому казалось, что фикс v0.46.4 не сработал. Теперь время `· N мин` дописывается во всех ветках subtitle одинаково.

---

## v0.46.4 — 2026-05-06 — Время в карточках приложений по фильтру день/неделя

### Улучшения

- **Время использования в карточках приложений зависит от выбранного фильтра.** На странице «Родительский контроль» (web + mobile-parent через WebView) переключение «Сегодня / Вчера / Неделя» теперь меняет не только график и сводку, но и подпись `· N мин` в каждой карточке приложения — раньше там всегда показывалось время за «сегодня», независимо от таб'а. Сортировка списка тоже теперь по убыванию активности за выбранный диапазон: hardcoded-системные сверху → дальше самые «горячие» приложения первыми → алфавитно среди равных.

### Изменения

- **CLAUDE.md:** зафиксировано правило «не работать в git worktree» — все правки только в основном чекауте `D:/Project/GMD/`, чтобы агент видел untracked-работу пользователя и не принимал её за «фантомные заглушки».

---

## v0.46.3 — 2026-05-03 — Удаление неиспользуемого PIN-функционала

### Изменения

- **Снят PIN родителя.** Страница `/cabinet/pin`, пункт «Задать PIN» в профиль-меню кабинета и REST-эндпоинты `/me/pin*` (включая verify) удалены. PIN был введён в v0.27.0 как L2-защита от деактивации Device Admin на устройстве ребёнка, отменён в v0.29.2 (защита держится только на Device Admin); код, страница и колонки `users.pinHash`/`pinUpdatedAt` стояли неиспользуемыми и сбивали с толку — текст на странице обещал «подтверждение защитных действий», ни одно из которых PIN не запрашивало. Backend: удалены `PinService`, `PinVerifiedGuard`, `UserPinService`/`UserPinController`, endpoint `POST /child/protection/verify-pin`, поле `hasPin` из `/me`. БД: миграция `20260503130000_drop_user_pin` дропает `users.pinHash` и `users.pinUpdatedAt`.

---

## v0.46.2 — 2026-04-30 — Защита от удаления + понятные push-уведомления о геозонах

### Новые возможности

- **Тумблер «Защита от удаления» на экране ребёнка mobile-parent.** Раньше переключатель в развёрнутом sheet'e был заглушкой и показывал «на следующем этапе». Теперь — функциональный: тап → AlertDialog с предупреждением (для ON: «Ребёнок не сможет удалить или отключить приложение GMD на своём устройстве»; для OFF: «Ребёнок сможет удалить приложение»), на confirm — busy-state spinner, PATCH `/family/children/:id/protection`, toast о результате. Disabled когда у ребёнка нет привязанного устройства. Этап 2 из 7 плана редизайна child-detail (см. `docs/engineering/plans/2026-04-29-child-detail-redesign.md`).

### Улучшения

- **Push-уведомления о геозонах теперь содержат имя ребёнка и название зоны.** Вместо безличного «Ребёнок вошёл в одну из геозон» родитель видит «Степан вошёл в зону «Школа»» с заголовком «Вход в зону «Школа»». Backend в FCM payload теперь резолвит `childName`/`zoneName` из БД, mobile-parent (ParentFirebaseMessagingService.kt) использует их в title/body. Если backend по какой-то причине не передаст имена — fallback на старый текст.

---

## v0.46.1 — 2026-04-30 — Hotfix: список детей не загружался после ночного простоя

### Исправления

- **Список детей в mobile-parent больше не падает с ошибкой подключения после долгого фонового простоя.** Воспроизведение: оставить приложение в background на ночь (Doze + разряд батареи), утром открыть → бесконечный спиннер или `Connection refused` с непонятным портом (типа `port=41790`). Причина — Android Doze разрывает TCP keepalive у idle-сокетов, но Dart `HttpClient` не уведомляется и пытается переиспользовать мёртвый socket из connection pool. Решение в три слоя: (1) интерсептор в Dio на любую `connectionError`/`SocketException` пересоздаёт `HttpClient` adapter и делает 1 retry, (2) `WidgetsBindingObserver` при возврате app из background после ≥5 минут idle проактивно сбрасывает pool до первого запроса, (3) pull-to-refresh на главном экране как ручная подстраховка.
