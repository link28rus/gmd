# Политика конфиденциальности Перископ

**Версия:** 1.6
**Дата вступления в силу:** 15 мая 2026 г.

Настоящая политика конфиденциальности (далее — «Политика») регулирует обработку персональных данных пользователей сервиса Перископ (далее — «Сервис», `periscop.pro`) в соответствии с Федеральным законом от 27.07.2006 № 152-ФЗ «О персональных данных».

Перископ — сервис родительского контроля и геолокации детей. Состоит из веб-кабинета родителя (`periscop.pro`), мобильного приложения родителя (Android, iOS) и мобильного приложения ребёнка (Android). Распространяется через RuStore (`https://www.rustore.ru`) и через прямую загрузку APK с сайта Сервиса.
...
Приложение Перископ на устройстве ребёнка запрашивает следующие разрешения Android. Все они обязательны для работы заявленных функций; запрос инициируется только после первичного подключения устройства родителем по QR-коду и явного шага в onboarding.
...
- **Что видно на устройстве ребёнка.** Когда приложение Перископ установлено и активно, на устройстве ребёнка постоянно отображается foreground-уведомление с минимальным приоритетом — «periscop · Готов к аудиомониторингу». Это обязательное требование Android 14 и более новых версий для приложений, использующих микрофон в фоне. В момент активной сессии прослушивания дополнительно работает системная индикация Android 12 и новее: в строке состояния появляется индикатор использования микрофона (зелёная точка/значок микрофона), отображаемый операционной системой при доступе любого приложения к микрофону. Это поведение Android, единое для всех приложений, и Оператор не имеет возможности его отключить.
- **Отдельного push-уведомления о начале или окончании конкретной сессии аудиомониторинга приложение Перископ не показывает.** Это сознательное конструкторское решение, направленное на обеспечение безопасности ребёнка в соответствии с упомянутой выше правовой позицией Конституционного Суда РФ.
...
- **Назначение:** реализация функции «Блокировка приложений» и расписаний автоблокировки. При попытке ребёнка открыть запрещённое родителем приложение Перископ показывает поверх него полноэкранный экран блокировки с таймером.
...
- **Назначение:** защита приложения Перископ от удаления ребёнком без согласия родителя. Без этого разрешения ребёнок может удалить приложение из лаунчера в один тап и обойти родительский контроль.
- **Что НЕ делается:** удалённая блокировка экрана устройства, сброс данных, изменение пароля экрана блокировки, навязчивые ограничения политик пароля. Перископ использует Device Admin исключительно для защиты от удаления.
...
- **Назначение:** автоматическая установка обновлений приложения. Используется только при прямой доставке APK с серверов Перископ (sideload-канал), необходимой при установке вне магазина приложений. При установке и автообновлении через RuStore это разрешение приложением не используется и системой не запрашивается.
...
Оператор вправе изменять настоящую Политику. Актуальная версия всегда публикуется по адресу `https://periscop.pro/privacy`.
